...
namespace yprfj
{
    public partial class Formerrorlog : Form
    {
        ...
        private void btnquery_Click(object sender, EventArgs e)
        {
            ...
            Sqlstr = string.Format("select * from VALARMS where convert(varchar(10),AlarmTime,23) between '{0}' and '{1}'", dateTimePicker1.Value.ToString("yyyy-MM-dd"), dateTimePicker2.Value.ToString("yyyy-MM-dd"));
            //MessageBox.Show(Sqlstr);
            try
            {
                ResDS = Database.GetDataset(Sqlstr, "VALARMS");
